 ...
 * ①链表是以结点的方式来储存，是链式存储
 * ②结点由数据域（你要存储的数据）和指针域（指向下个结点的地址）构成
 * ③链表的各个结点不一定是连续存放的
 * ④链表分带头结点（头结点为空）的链表，根据实际的需求来确定。
 *
...
public class SingleLinkedList {
     ...
     * 删除结点
     * head不能动
     * 我们比较时, 是temp.next.no和需要删除的结点的no比较
     * @param no
     */
    ...
    /**
     * 倒数第k个结点
     * (快慢指针: 我们把一个链表看成一个跑道，假设a的速度是b的两倍，那么当a跑完全程后，b刚好跑一半，以此来达到找到中间结点的目的。)
     *
    ...
    public void reverse() {
        ...
        while (cur != null) {
            ...
            cur.next = reverseHead.next;   // 新结点插入到头结点后面, 即下一个结点为头结点后面的
            reverseHead.next = cur;        // 新结点插入到头结点后面, 即temp下一个结点为cur
            cur = next;                    // 让cur后移
        }
    ...
    /**
     * 迭代反转-3引用
     * 所谓的单链表反转，就是把每个结点的指针域由原来的指向下一个结点变为指向其前一个结点。但由于单链表没有指向前一个结点的指针域，
     * 因此我们需要增加一个指向前一个结点的指针pre，用于存储每一个结点的前一个结点。此外，还需要定义一个保存当前结点的指针cur，
     * 以及下一个结点的next。定义好这三个指针后，遍历单链表，将当前结点的指针域指向前一个结点，之后将定义三个指针往后移动，直至遍历到最后一个结点停止
     * @return
     */
    public void iteratorReverse() {
        ...
        while (curNode != null) {
            nextNode = curNode.next;  // nextNode 指向下一个结点, 引用curNode.next的内存地址
            curNode.next = preNode;   // 将当前结点next域指向前一个结点, curNode的next指向的内存地址置空, 但是next的对象还在, 正在被nextNode引用
